        ...
        <div class="row">
            <div class="col-12">
                <form method="POST" action="registration.php">
                    ...
                    <button type="submit" class="btn_red btn_reg" name="submit" style="margin-top:20px">Продолжить</button>
                </form>
            </div>
        </div>
    </div>
...
if (isset($_POST['submit'])) {
    ...
    if (!$email || !$username || !$pass) die ('<div style="text-align: center;"><p>Пожалуйста введите все значения</p></div>');
    $sql = "INSERT INTO users (email, username, pass) VALUES ('$email', '$username', '$pass')";
    if(!mysqli_query($link, $sql)) {
        echo "<div class='text-center'>Не удалось добавить пользователя</div>";
    }else{
